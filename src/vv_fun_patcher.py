...
VV5_ORIGINS_RELOCATION_LEDGER_SHA256 = (
    # The expanded-256 relocation ledger is removed; this is the canonical
    # digest of an EMPTY row list, so the check below now asserts the ledger
    # stays gone rather than staying frozen at 66 rows.
...
VV3_RUNNING_CERTIFIED_SHA256 = {
    ...
    "map": "533C75C2D8E176A3EAC7B01E1A1DA747D90415F11799CDB90B16247F359D54B5",
}
...
VV2_FULL_MASTERY_MANIFEST_SHA256 = "FE1215A105C1EA20B50ACAA44FFCB17870C133AA8A58E106AE9081718E346B86"
VV2_FULL_MASTERY_MAP_SHA256 = "7B189CAED82C469851C9C866AE1452B79F8514CDA950A2500AEE6C183D3579FA"
VV2_FULL_MASTERY_CERTIFIED_SHA256 = {
    ...
    "dll": "38117D882EC3A4E095CA02A606049521B7842280CF988FE3E00389017FAF7535",
}
...
VV2_INDIVIDUAL_FULL_MASTERY_MANIFEST_SHA256 = "A2E5BDB06F3D473E55BB1CBE7EF23E755145C1755B65A0B40AFB03B755EE9FEA"
VV2_INDIVIDUAL_FULL_MASTERY_MAP_SHA256 = "646491CFC46C5639D770F6A92A421847643B3B8EAA1642C0EA340FE9D87CB49A"
...
VV4_FULL_MASTERY_CERTIFIED_SHA256 = {
    ...
    "dll": "1AE5EF5F319491F4FCD927D232F669A76E1E15EB360B1E0F97055EF7EC14561C",
}
...
VV4_FULL_HEAL_PARENT_DLL_SHA256 = "1AE5EF5F319491F4FCD927D232F669A76E1E15EB360B1E0F97055EF7EC14561C"
...
VV5_FULL_MASTERY_CERTIFIED_SHA256 = {
    ...
    "dll": "D836427D743B5B4D1CE93569E29F5C0DE64289D6E720FF01048A7783D2EF15BF",
...
VV5_TASK9_SOURCE_TEXT_SHA256 = {
    "manifest": "A203C44C171F6DF38BF543E59743A2B80067ECB9C0287D92A7976E76C2BF4298",
    "map": "533C75C2D8E176A3EAC7B01E1A1DA747D90415F11799CDB90B16247F359D54B5",
}
...
VV5_TASK9_SAVE_RESET_SHA256 = "3AE4ABA3EA06CA9F62D5A37CF68C2786C23FDC370C6FA387134287BB344683D9"
...
VV5_TASK9_ACTIVE_SOURCE_TEXT_SHA256 = "8021C96CCAB8D03A54435A4CEDC71EEA146E06F8C90F65B451C400875445C1BD"
...
EXPANDED_TIME_WARP_SOURCE_TEXT_SHA256 = {
    ...
    "task9_builder": "0879FC91AB66E9816C8F7AD86BFE3518ED4898AFB12C3217EDD47685F3F075A3",
}
EXPANDED_TIME_WARP_ARTIFACT_SHA256 = {
    ...
    "vv5": {
        "manifest": "0422836DE9BD46143A7E88DFA09CA1956F7FFAA68892D0B1480E18FB9E9BF6E9",
        "map": "64F46D2CB70BADBD81F8FE87630A183AF7B178A98A132E110AE567B7B6583117",
    },
}
...
def _certified_vv5_task9_record(active_base: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        or len(task9_relocation) != 0
        or relocation_digest != VV5_ORIGINS_RELOCATION_LEDGER_SHA256
        or record.get("frozen_c342")
        != {"count": 0, "rows_sha256": VV5_ORIGINS_RELOCATION_LEDGER_SHA256, "unchanged": True}
    ):
    ...
    *, include_expanded_time_warp: bool = False
    ...
    for feature_path in NUMERIC_KEYS_FEATURE_PATHS:
        if feature_path.is_file():
            ...
            if record.get("enabled", True):
                items.append(record)
    for feature_path in SHOW_PARENTS_FEATURE_PATHS:
        if feature_path.is_file():
            record = json.loads(feature_path.read_text(encoding="utf-8"))
            if record.get("enabled", True):
                items.append(record)
...
def _relocate_composition_overlay_hooks(
    ...
    for feature in fun_patches:
        ...
        if base_id is not None:
            # The base is looked up in the catalog when it is not in the list
            # passed here, because REMOVAL passes only the feature being
            # removed while still naming the base in selected_ids. Resolving
            # it from the list alone raised a bare StopIteration -- an
            # exception carrying no message at all -- so uninstalling a
            # composed feature failed while saying nothing about why.
            base = next(
                (item for item in fun_patches if item.id == base_id), None
            )
            if base is None:
                try:
                    base = get_fun_patch(base_id)
                except (PatcherError, KeyError):
                    # A declaration naming a feature that does not exist is a
                    # manifest defect, not a reason to write the standalone
                    # bytes over an overlay install.
                    raise PatcherError(
                        f"{feature.name} ({feature.id}) declares a composition "
                        f"overlay on {base_id}, which is not a known feature."
                    ) from None
            base_layout = _append_layout(base, patch_mode, selected_ids)
...
def _remove_feature_bytes(
    ...
    if composition_overlay is None:
        # The same detection for any other feature that declares an overlay.
        #
        # Removal is handed only the feature being removed, so it cannot know
        # from the request which form was installed -- and it must undo the one
        # that actually is. The form is therefore read back out of the image:
        # an overlay is present exactly when its generated bytes are sitting at
        # its declared offset. Guessing instead would reverse the standalone
        # hook over an overlay install, fail the preimage guard, and leave the
        # hook pointing into a page that removal is about to truncate away.
        transaction = feature.raw.get("pe_append_transaction")
        overlays = (
        ...
        if isinstance(overlays, dict):
            for base_id, overlay in overlays.items():
                ...
                try:
                    ...
                    continue
                if start < 0 or length <= 0 or start + length > len(work):
                    continue
                candidate = {**overlay, "_composition_overlay": True}
                try:
                    page = _resolve_append_bytes(feature, candidate)
                except PatcherError:
                    continue
                if bytes(work[start : start + length]) == page[:length]:
                    composition_overlay = (candidate, page)
                    break
...
VV5_INDIVIDUAL_RUNNING_MANIFEST_SHA256 = "895FE6F51E4753B8F9FB0535CCD63C0B0BFAD525E151DC7FE341E629932F9526"
VV5_INDIVIDUAL_RUNNING_MAP_SHA256 = "C6297EF0BAA975CC2CA605A9885C3CE4A8FB6449F77ACC72157F34F50B1440C0"
