...
That is not hypothetical. When the logs moved into `Virtual Villagers Fun Patcher Logs\<kind>\`
...
def subfolders(source: str) -> set[str]:
    r"""Every `Virtual Villagers Fun Patcher Logs\<kind>` literal a file names, narrow or wide."""
    return set(re.findall(r'Virtual Villagers Fun Patcher Logs\\\\([A-Za-z ]+)', source))
...
class HarnessesFollowTheirExportersTests(unittest.TestCase):
    def test_the_parentage_harness_reads_the_parentage_folder(self):
        ...
        self.assertTrue(
            exporter, "the exporter names no Virtual Villagers Fun Patcher Logs subfolder at all")
        self.assertTrue(
            harness,
            "the harness names no Virtual Villagers Fun Patcher Logs subfolder, so it is reading the "
    ...
    def test_the_population_harness_reads_a_population_folder(self):
        ...
        self.assertTrue(exporter, "the exporter names no Virtual Villagers Fun Patcher Logs subfolder")
        self.assertTrue(harness, "the harness names no Virtual Villagers Fun Patcher Logs subfolder")
        self.assertTrue(
            harness <= exporter,
            "the harness reads %s but the exporter writes %s"
            % (sorted(harness), sorted(exporter)))
