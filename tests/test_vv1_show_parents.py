...
class RecoveryAndFallbackTests(unittest.TestCase):
    """The retroactive recovery, the Unknown/0/0 fallback father, and the
    father's age at conception -- the parts that reconstruct or complete
    parents from the log the game already wrote."""

    def test_export_dll_exports_the_recovery(self):
        import pefile  # noqa: PLC0415
        dll = pefile.PE(str(ROOT / "assets" / "parentage" / "VVFP Parentage Export.dll"))
        names = {e.name.decode() for e in dll.DIRECTORY_ENTRY_EXPORT.symbols if e.name}
        self.assertIn("RecoverParentageParents", names)
        d = (ROOT / "native" / "parentage_export" / "parentage_export.def").read_text(encoding="utf-8")
        self.assertIn("RecoverParentageParents=_RecoverParentageParents@12", d)

    def test_recovery_uses_the_same_layout_index_as_the_writer(self):
        """The off-by-one that shipped once: RecoverParentageParents must index
        GAME_LAYOUTS the same way WriteParentageRecordWithFather does (by
        game_id, not game_id - 1; slot 0 is unused)."""
        export = EXPORT_C.read_text(encoding="utf-8")
        self.assertNotIn("GAME_LAYOUTS[game_id - 1]", export)
        self.assertEqual(export.count("GAME_LAYOUTS[game_id]"), 3)

    def test_recovery_pairs_birth_to_conception_and_falls_back(self):
        export = EXPORT_C.read_text(encoding="utf-8")
        self.assertIn("!c_used[j] && strncmp(c_mother[j], b_mother[i], RECOVER_NAME_CAP) == 0", export)
        self.assertIn("reqs[r].mother", export)
        self.assertIn('#define RECOVER_FALLBACK_NAME "Unknown"', export)
        self.assertIn("#define RECOVER_FALLBACK_HEAD 0", export)
        self.assertIn("#define RECOVER_FALLBACK_BODY 0", export)
        self.assertIn("if (reqs[r].found ||", export)     # a request already resolved is skipped
        # a spawn (no logged birth) is left with found==0 -> the parentage DLL
        # gives it no parents; that spawn-skip lives in vv1_recover_from_log
        parentage = PARENTAGE_C.read_text(encoding="utf-8")
        self.assertIn("if (!reqs[i].found)", parentage)

    def test_recovery_keeps_a_conception_for_its_whole_litter(self):
        """Twins and triplets (all five games): one conception, N births.  The
        conception must be consumed only after all N babies are paired, or the
        second and third child get the Unknown fallback instead of the real
        father.  Regression guard for the P1 that shipped in the first draft."""
        export = EXPORT_C.read_text(encoding="utf-8")
        # the "Babies in pregnancy" count is parsed and clamped to 1..3
        self.assertIn('recover_field(line, "Babies in pregnancy:", value, sizeof value)', export)
        self.assertIn("cur_babies = (b >= 1 && b <= 3) ? b : 1", export)
        # a conception is consumed only after that many births match it
        self.assertIn("if (++c_matched[match] >= c_babies[match]) {", export)
        self.assertIn("c_used[match] = 1;", export)
        # the old bug -- consume on the first match unconditionally -- is gone
        self.assertNotIn("if (match >= 0) {\n            c_used[match] = 1;\n        }", export)

    def test_recovery_only_reads_the_current_villages_logs(self):
        """A second village's log in the same save folder must not hand a child
        another village's parents.  Recovery applies the same village-header
        filter the log writer's select_log_file uses.  Regression guard for the
        cross-village P1."""
        export = EXPORT_C.read_text(encoding="utf-8")
        self.assertIn("vv_village_recall(village, sizeof village)", export)
        self.assertIn("if (!log_belongs_to_village(path, village)) {", export)

    def test_recovery_disambiguates_same_named_children_by_appearance(self):
        """Two children sharing a name must not swap parents.  The birth row
        carries the child's head/body, and the caller passes the living child's,
        so a birth is matched on name AND appearance (a caller that does not
        know the appearance passes -1 and matches by name).  Regression guard
        for the same-name P1."""
        export = EXPORT_C.read_text(encoding="utf-8")
        # the request carries the child's head/body as inputs
        self.assertIn("int child_head;", export)
        self.assertIn("int child_body;", export)
        # the pairing skips a birth whose appearance does not match, when known
        self.assertIn("if (reqs[r].child_head >= 0", export)
        self.assertIn("reqs[r].child_head != b_chead[i] || reqs[r].child_body != b_cbody[i]", export)
        # the caller (parentage DLL) supplies the living child's head/body, and
        # its struct mirrors the export's ABI
        parentage = PARENTAGE_C.read_text(encoding="utf-8")
        self.assertIn("int child_head, child_body;", parentage)
        self.assertIn("reqs[count].child_head = *(const int *)(rec + VV1_HEAD_OFFSET);", parentage)

    def test_recovery_keeps_the_most_recent_births_when_overflowing(self):
        """A village past RECOVER_MAX births must not lose its recent (living)
        newborns: the parser drops the OLDEST row, not the newest.  Regression
        guard for the P2 buffer-cap finding."""
        export = EXPORT_C.read_text(encoding="utf-8")
        self.assertIn("recover_push_birth(", export)
        # on overflow it shifts the oldest out, keeping the most recent
        self.assertIn("if (*n_birth >= RECOVER_MAX) {", export)
        self.assertIn("*n_birth = RECOVER_MAX - 1;", export)

    def test_vv3_conception_trampoline_reads_the_shifted_mother_slot(self):
        """VV3's trampoline pushes the father (saved EDI) first, dropping ESP by
        four, so the mother (saved ESI) is then at [esp+0x08], not [esp+0x04].
        Reading +0x04 there re-pushes the father and logs him as the mother.
        Regression guard for that P1, checked in the generated bytes."""
        try:
            import capstone  # noqa: PLC0415
        except ImportError:
            self.skipTest("capstone not installed")
        source = (ROOT / "scripts" / "build_vv3_parentage_feature.py").read_text(encoding="utf-8")
        self.assertIn("push dword ptr [esp + 0x00]\n            push dword ptr [esp + 0x08]", source)
        self.assertNotIn("push dword ptr [esp + 0x00]\n            push dword ptr [esp + 0x04]", source)
        # and in the emitted append_bytes: push [esp]; push [esp+8] appear in order
        manifest = json.loads((ROOT / "data" / "vv3_parentage_feature.json").read_text(encoding="utf-8"))
        blob = bytes.fromhex(manifest["features"][0]["pe_append_transaction"]["layouts"]["immediate_fixed"]["append_bytes"])
        md = capstone.Cs(capstone.CS_ARCH_X86, capstone.CS_MODE_32)
        md.skipdata = True
        seq = [f"{i.mnemonic} {i.op_str}" for i in md.disasm(blob, 0)]
        joined = " | ".join(seq)
        self.assertIn("push dword ptr [esp] | push dword ptr [esp + 8]", joined)

    def test_parentage_dll_recovers_partial_and_never_overwrites(self):
        parentage = PARENTAGE_C.read_text(encoding="utf-8")
        self.assertIn('GetProcAddress(companion, "RecoverParentageParents")', parentage)
        self.assertIn("g_entries[i].mother_head != 0 && g_entries[i].father_head != 0", parentage)
        self.assertIn("if (g_entries[j].mother_head == 0)", parentage)
        self.assertIn("if (g_entries[j].father_head == 0", parentage)

    def test_the_fallback_father_is_unknown_zero_zero(self):
        parentage = PARENTAGE_C.read_text(encoding="utf-8")
        self.assertIn('#define VV1_FALLBACK_FATHER_NAME "Unknown"', parentage)
        self.assertIn("vv1_plus_one(0)", parentage)
        self.assertIn("VV1_FALLBACK_FATHER_NAME, VV1_NAME_CAPACITY", parentage)
        self.assertEqual(parentage.count("vv1_set_father("), 3)

    def test_father_age_at_conception_is_logged_from_his_own_record(self):
        export = EXPORT_C.read_text(encoding="utf-8")
        self.assertIn(r'"    Age at conception: %s\n"', export)
        self.assertIn("*(const int *)(father + g->age)", export)
        self.assertIn("father_age", export)
        self.assertIn('memcpy(father_age, "(not captured for this birth)", 30)', export)

    def test_the_parentage_log_format_the_parser_reads_matches_the_writer(self):
        export = EXPORT_C.read_text(encoding="utf-8")
        for label in (r'"Conception %d\n"', r'"  Mother: %s\n"', r'"  Father: %s\n"',
                      r'"  Child: %s\n"', r'"    Head: %s\n"', r'"    Body: %s\n"'):
            self.assertIn(label, export)
        for marker in ('"Conception "', '"Birth"', '"Child:"', '"Mother:"',
                       '"Father:"', '"Head:"', '"Body:"'):
