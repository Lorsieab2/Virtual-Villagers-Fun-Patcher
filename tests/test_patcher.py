...
class ManifestTests(unittest.TestCase):
    ...
    def test_statistics_exporter_recovers_completed_vv5_bonus_puzzle_from_save(self) -> None:
        ...
        # The description is GENERATED by scripts/build_statistics_features.py,
        # so pin the promise rather than a phrasing the generator may reword.
        # It used to read "including current puzzle totals"; the generator now
        # says puzzle totals "are read from the current save state during
        # export", which is the same guarantee stated more precisely.
        descriptions = " ".join(
            patch.description
            for patch in load_fun_patches()
            if patch.name == "Write Village Statistics to Text File"
        )
        self.assertIn("puzzle totals are read from the current save state",
...
class GuiSourceTests(unittest.TestCase):
    ...
    def test_success_confirmation_uses_clear_folder_links(self) -> None:
        ...
        self.assertIn("Births and Conceptions Log N.txt:", source)
