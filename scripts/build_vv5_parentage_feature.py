...
It used to replace the CALL to sub_465E00 in the role resolver (0x467DBE),
so only that caller was logged -- autonomous embracing (0x46E16B / 0x46E19B, the two
...
0x465F34 is that exit: all three litter outcomes converge on it after the litter
size is final, and the capacity rejection jumps past it to 0x465F44. No branch
or stored pointer targets the ten bytes taken there
(`test bl,bl / jne 0x465F44 / mov ecx,[esi+0x1C50]`); the page replays them
and jumps back to 0x465F3E.
...
sub_465E00's last stack argument gates its statistics block.  Village seeding
calls it with that argument SET and a hardcoded father "Joey":

    @ 0x471B6D    seeding, suppression on
