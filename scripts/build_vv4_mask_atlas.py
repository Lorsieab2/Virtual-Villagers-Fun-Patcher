...
def _place(mask: Image.Image, face_cx: float, face_cy: float) -> Image.Image:
    ...
    The output is the head cell, so anything beyond it clips (the cell is 40x65)."""
...
def build_atlas(src: Image.Image, native: list[list[Image.Image]]) -> Image.Image:
    w = src.size[0]
    ...
    for r in range(MASK_ROWS):
        for f in range(FRAMES):
            fcx, fcy = faces[f]
            out.paste(_place(native[r][f], fcx, fcy), (f * CELL_W, CELL_H * (HEAD_ROWS + r)))
    return out
