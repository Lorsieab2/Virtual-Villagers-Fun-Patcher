...
# Island queue: the same, for the purchased island event.  The village field at
# [world + 0x12EF4] still carries the due stamp for the GAME's own scheduling,
# but the handler hook runs in a frame context that has no pointer to the world
# -- edi/ebp there belong to the purchase menu, not to this handler -- so the
# release check reads its own copy by absolute address, exactly as the barrel's
# does.
ISLAND_DUE_VA = SECTION_DATA_VA + 0x50
...
# ---- Village / world Heathen-mask hook (replay the authoritative head tuple) ----
# VV3's per-villager handler sub_4605F0 has two nearby sprite painters.  The old
# 0x460A60 -> 0x42E570 hook used a generic fixed-resource draw that does not consume
# scale for +0xF12 held records, explaining the tiny ground/pickup ghost.  It is
# not a valid mask hook.  The authoritative appearance call is 0x460C7F ->
# 0x42E5E0: stock loads record+0xDF0 (head atlas), record+0xF18 (facing), and
# supplies six arguments (atlas, x, y, head-row, facing, scale).  The cave below
# re-issues that exact call, then calls VV3WorldMaskDrawAt(record, &args).  The
# callback immediately calls the SAME 0x42E5E0 with only atlas and row replaced,
# preserving stock coordinates, facing, scale, alpha, and manager state.  Held
# records fail closed because this site is proven to retain the old ground tuple;
# no cursor-leaf tuple is claimed.  DLL publishes its callback in DllMain.
WORLD_MASK_CAVE_VA = SECTION_CODE_VA + 0x000   # appended R-X section
WORLD_MASK_CALLSITE_VA = 0x460C7F         # sub_4605F0's authoritative `call sub_42E5E0` (head)
WORLD_HEAD_DRAW_VA = 0x42E5E0             # appearance draw; tuple is atlas,x,y,row,facing,scale
WORLD_DRAWFN_PTR = SECTION_DATA_VA + 0x04               # DLL publishes VV3WorldMaskDrawAt here
...
# The inline mask draw is deliberately before any later full-body action overlay,
# so stock action rendering can naturally occlude it.  No mask or loader hook is
# installed at 0x42E3F5; the head cave lazy-loads the companion before the stock
# appearance call and then runs the mask synchronously.
...
# The purchased Island Event needs the same kind of dedicated pending flag the
# barrel already has.  A timestamp comparison cannot serve as one: if the delay
# elapses before the handler consumes the queue -- the player reopens the paused
# Tech menu during the window and leaves it open -- `stamp - now` underflows as
# unsigned and the still-queued event reads as not pending, so a second purchase
# deducts another 30,000 points and overwrites the outstanding event.  Passing
# the deadline does not prove consumption, so pending is tracked explicitly and
# cleared only where the event is actually presented.  0x4B3C76 is verified
# free: zero in the stock image, referenced by no instruction in .text, and
# below the event-object array that starts at 0x4B3C78.
ISLAND_PENDING_FLAG_VA = 0x4B3C76
...
# Releasing the island pending flag needs a clock call, a compare and a
# store -- nine bytes more than the 0x7B3B1 frame hook has, and growing that
# hook ran into barrel_present at 0x7B3E0 (the byte guard caught it).  It
# cannot live inside barrel_present either: that routine only runs when a
# BARREL is pending, so the island flag would stick and block every later
# Island Event purchase.  It therefore gets its own slot in the patch-owned
# page -- which is what that page is for -- and the hook reaches it with a
# single five-byte call.
ISLAND_RELEASE_CAVE_OFF = 0x380
ISLAND_RELEASE_VA = SECTION_CODE_VA + ISLAND_RELEASE_CAVE_OFF
...
def main() -> None:
    ...
    put(
        ...
        legacy_not_time_warp:
            ...
            #
            # Pending is a dedicated flag, not a timestamp comparison.  The
            # earlier revision of this guard read the due stamp into edx and
            # subtracted the clock from it, which was wrong twice over: edx is
            # caller-saved and 0x403330 writes it (verified by disassembly --
            # `mov edx, [esp+4]` at 0x403341, alongside eax and ecx), so the
            # subtraction consumed callee scratch; and an elapsed deadline was
            # treated as proof the event had been consumed, which it is not.
            # The flag is set when the event is armed and cleared only where it
            # is actually presented, exactly as the barrel's flag works.
            #
            # Only the pending case refuses.  The not-pending case must FALL
            # THROUGH to ie_charge_ok so the purchase is charged and queued --
            # an earlier revision jumped both branches to the refusal, which
            # made ie_charge_ok unreachable for ebx == 1 and refused every
            # Island Event purchase.
    ...
    put(
        MASK_CAVE_VA,
        f"""
            # This is a JMP from the post-push call site at 0x456B24, not a
            # CALL into the cave: at entry [esp+0]..[esp+0x18] are arg1..arg7.
            # Repeating [esp+0x18] therefore duplicates arg7..arg1 as pushes.
        ...
        mask_have:
            # 0x409FB0 ret 0x1C restores esp to the first original argument.
            mov edx, esp
            push edx
    ...
    # Head-site tuple cave: at 0x460C7F the game does `call 0x42E5E0` with esi=record and
    # six appearance args on the stack ([esp+4]=atlas,+8=x,+0xC=y,+0x10=row,+0x14=facing,
    # +0x18=scale).  Redirect that call here: re-issue the identical call with a COPY of the
    # six args (sub_42E5E0 is ret 0x18 so it cleans the copies, leaving the originals), then
    # hand the DLL VV3WorldMaskDrawAt(record,&origArgs) the complete authoritative tuple.
    # The callback draws immediately through 0x42E5E0, changing only atlas and row and
    # preserving stock facing/position/scale.  Later stock action overlays can naturally
    # cover this inline head layer.  Null ptr -> plain head. esi + callee-saved regs survive.
    world_mask_cave = assemble(
        f"""
            cmp dword ptr [0x{WORLD_DRAWFN_PTR:X}], 0
            jne world_companion_ready
            push 0x{s['icons_dll']:X}
            call dword ptr [0x47C124]
        world_companion_ready:
            push dword ptr [esp + 0x18]
            push dword ptr [esp + 0x18]
            push dword ptr [esp + 0x18]
            push dword ptr [esp + 0x18]
    ...
    # No held/effect/action cave is assembled; stock full-body action rendering stays untouched.
    ...
    # Clear the island pending flag once the queued event's delay has
    # actually elapsed, so a later purchase is allowed again.  Runs every
    # frame from the island-event handler splice.  eax/ecx/edx are all
    # caller-saved and the clock writes all three, so they are saved here
    # rather than relying on the caller: the spliced tail reloads ecx from
    # [esi+0x10], but nothing guarantees the rest.
    island_release_code = assemble(
        f"""
            cmp byte ptr [0x{ISLAND_PENDING_FLAG_VA:X}], 0
            je ir_done
            push eax
    ...
    put_cave(
        ISLAND_RELEASE_CAVE_OFF,
        island_release_code,
        "release the island pending flag once its delay has elapsed",
    )
    ...
    # The Details cave remains in the payload; the world tuple cave is emitted into
    # the appended R-X section below.
    patch(
    ...
    patch(
        ...
        "redirect the authoritative 0x42E5E0 head draw through an inline six-argument mask cave",
    )
    put_cave(0x000, world_mask_cave, "inline world mask cave: replay stock six-argument head tuple, then overlay synchronously")
    put_cave(
        0x100,
    ...
    #   * the old fixed-context holder [+0x127C1C] is read at EXACTLY ONE site in the whole
    #     exe (0x460A54, feeding the disproven generic draw at 0x460A60); the authoritative
    #     record appearance fields [+0xDF0]/[+0xF18] are loaded for the head call at 0x460C7F;
