...
def build_document() -> str:
    ...
    lines = [
        ...
        "VV4 Full Mastery UI Playtest 3 is HARD WITHDRAWN and both its base and individual Full Mastery records are catalog-hidden after a 0xC0000005 crash at RVA 0x89E0C / VA 0x489E0C. The individual-menu result calls at 0x4897CA and 0x489ABB incorrectly targeted the show-menu epilogue at 0x489573 instead of the result helper at 0x489583; its `ret 8` consumed the result-message pointer as the return address. The disabled repair candidate changes only those guarded call targets. Individual Full Mastery remains STOP because its command writes raw Float32 90 and precharges, while all-villager command 7 is separately withheld pending D25 recertification. The native trophy UI repair, Cure containment, unrelated features, and Expanded-256 ON HOLD are unchanged.",
        "",
