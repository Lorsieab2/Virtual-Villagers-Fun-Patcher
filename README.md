...
`vv3_all_villagers_like_running` feature is enabled for runtime playtesting
after final certification `c62fba9214de7c6092365e99c72bd81a59d3888c`;
gameplay validation remains pending and commands 7/8 remain absent. VV4's
...
VV5's independent command-7-only Full Mastery package at commit `5e52be5` is
HARD WITHDRAWN after an immediate startup auto-close. Certification `8193629`
is revoked. The disabled corrective candidate fixes the missing thiscall ECX
receiver in both base-owned constructors and remains catalog-hidden pending
independent emitted-byte recertification.
