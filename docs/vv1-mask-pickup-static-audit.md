...
`y = args[2] - (scale >> 3) - 15` and `x = args[1] + 1`. Screen Y grows downward, so the trailing
term seats every VV1 Details mask 15 pixels above the scale-aware
registration, one pixel to its right, while retaining that live scale
registration. The village mask path
