...
`271 * 0x3D8 + 0x58 + 25 * 0x18 = 0x41458`, the manager-to-object offset of
the corrupted field.

The fix is limited to that multi-creation path. The first-call trampoline
checks the live population count at `[manager+0x9E24]` and skips the first
creation when it is `>=255`; the second-call trampoline skips the second when
it is `>=256`. Both success paths call the unchanged stock creator, and both
skip paths clean the five stock arguments before returning through the stock
cleanup. The stock allocator's return contract and the other direct callers
are unchanged.

The generated safety rows are guarded by the exact stock preimages and use
zero-filled `.text` caves at file offsets `0x565E0` and `0x56840`. Static
