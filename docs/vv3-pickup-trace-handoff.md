...
- `0x42E3F5` is the sole direct caller of `sub_4605F0` in the stock executable
  and remains byte-identical in the candidate.
- The old `0x460A60 -> 0x42E570` hook was wrong: it uses a fixed context sprite,
  omits the record appearance fields, and halves scale for a held record. The
  authoritative stock appearance call is `0x460C7F -> 0x42E5E0`; stock loads
  record `+0xDF0`/`+0xF18` and supplies `(atlas, x, y, head-row, facing, scale)`.
  The candidate replays all six arguments, then calls the same draw immediately
  with only atlas and row changed.
- The action/full-body overlay is `sub_45F7E0`, reached at both `0x460B48` and
  `0x460D10`. Both calls remain stock; no unproved pose mask reconstruction is
  installed, so later action rendering can naturally cover the inline head mask.
- Stock drag sets `record+0xF12 = 1` while held, then the held branch rejoins the
  same body/head sequence and reaches `0x460C7F`. The inline callback therefore
  reuses the authoritative tuple while held; cursor coordinate ownership and
  final visual follow remain player-trace gates.
- The ordinary `record+0xF20` value is `-1`; an `+0xE8C` nonzero route can set
 ...
 - Action mask seating is not claimed for any `record+0xF20` value; stock owns
  all action poses while the runtime trace boundary remains open.
...
3. every `0x42E510`/`0x42E570` entry, including return address, atlas/cell,
   x/y, facing, scale, and whether the draw came from the stock head or action path;
4. the first release frame where `+0xF12` clears and the held identity/coordinates
