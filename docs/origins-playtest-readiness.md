...
available. Expanded composition is ON HOLD.

The expanded-256 relocation ledger has been REMOVED: its 66 rows existed only to relocate the payload for expanded-256, which is not a selectable patch mode and which no variant applies, while its hand-recorded byte snapshots blocked every legitimate payload edit. Nothing is relocated in its place, because there is no selectable mode
that would need it. The former ledger and the 43 current-feature references it
once recorded are historical only, per disassembly commit
