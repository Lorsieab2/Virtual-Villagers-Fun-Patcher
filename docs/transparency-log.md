...
- Evidence status: static source/manifest verification performed; runtime/player confirmation pending
- Guarded executable edits: 78; every edit has an exact purpose and before/after guard in the manifest.
...
- Behavior changes: The Details screen shows a "Sort By:" band with Age, Skill and Health plates and radios in the space between the Age/Gender boxes and the villager strip; clicking a plate or its radio chooses the order. The Details left/right arrows step through the living villagers in the chosen order (ascending key, record index among equals, wrapping at either end); the list position is kept across a change of order, as in the later games.
