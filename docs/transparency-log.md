...
VV4's isolated command-7 Full Mastery candidate uses the native Float32 writer to reach 100. The C6 direct-resource Tech/Detail UI candidate uses local x=72, y=4, exact 99x35 frames from the repository-owned mockup strip, removes the misidentified runtime dimension calls, and is disabled pending fresh independent recertification. Its withdrawn Cure row is rendered unavailable and command 5 is rejected before charge/0x728004 dispatch; the Cure payload bytes remain unchanged. Commands 6/8 and the legacy atomic village-wide payload remain contained, and Expanded-256 remains ON HOLD/fail-closed.
...
- Doubler purchase status: {'new_purchase': 'temporarily unavailable pending exact-build provenance verification', 'existing_owned': 'removable at zero cost with zero refund', 'repurchase': 'temporarily disabled pending exact-build provenance verification'}
- Evidence status: D13 independently recertified exact emitted bytes from e9afe69e0461ff986adddb55d743cf091eea598b; stock-mode playtest enabled; Expanded-256 ON HOLD/fail-closed
- Guarded executable edits: 12; every edit has an exact purpose and before/after guard in the manifest.
