...
VV4, and VV5 remain STOP until their own exact-build hook and provenance gates
are complete.
...
| VV3 The Secret City | `0x427130` / payload `tech_increment` | `0x4263F0` / payload `food_increment` | Exact build `8BC5DB382D02BC5C21AD5F607580D60FF44A6519CC7EB133F03113BAACAE6503` has no Food Mastery-like award transform in the writer, strings, or bounded caller corpus. Collection dispatcher `sub_42DEB0` awards tech 100/250/1,500 by the documented ID ranges and resolves writers at `0x42DF79` and `0x42E079`; IDA has no resolved caller to the dispatcher. | Complete inventory: food 33 rows (29 calls, E9 tails `0x415EF1`, `0x416983`, `0x416BAB`, `0x417A3A`); tech 16 rows (13 calls, E9 tails `0x415D44`, `0x41673E`, `0x418452`). Island Event positive/zero/negative/bypass paths are inventoried, but mixed-source writers have no source tag and `sub_458DB0` emits base and bonus components through separate tech-writer calls. No safe final-delta/source-aware hook, transient marker, or certified new section/cave is proven. | **STOP** |
...
| VV5 New Believers | `0x4237B0` / payload `tech_increment` | `0x41EB40` before storage/statistics channels | Exact build `92946781980220E9D1A2E6C573925519934608F5215F4A0F8CE3B90088C5C65D` (991,232 bytes) has Food Mastery tech ID 4: the upgrade from level 1 to 2 costs 3,000 and the upgrade from level 2 to 3 costs 40,000 tech points; positive A becomes A, A+floor(A/2), or 2A; zero/negative inputs bypass. Ordinary collection return `0x414970` maps base 6/35 to 6/35, 9/52, or 12/70. | The positive-whitelist runtime correction has a complete static GO specification: ordinary collection return `0x414970` is eligible, while Island Event, startup, consumption, and unknown callers remain native because unknown callers cannot match that return. The correction is not implemented in this docs-only turn; expanded-256 remains ON HOLD pending rel32 relocation coverage. | **GO specification; implementation pending** |
