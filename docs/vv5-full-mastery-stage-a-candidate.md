...
Each Tech and Detail modal call is wrapped by a guarded SDL2 fullscreen-desktop transition using GetKeyboardFocus RVA 0xA1910, GetWindowFlags RVA 0xA3E40, and SetWindowFullscreen RVA 0xA40E0; failed entry/restore is fail-closed. This remains a STOP: the native engine transition at VA 0x404700 and its screen-to-engine pointer chain are unproved, so leaving the game windowed after a restore failure is not accepted as release-safe and the candidate stays disabled/catalog-hidden.
